...
export default migration('warn SearchBox.onChange args changed', (opts: IMigrationOptions): ModResult[] => {
    ...
    if (
      ...
      for (const prop of node.attributes.properties) {
        if (ts.isJsxAttribute(prop) && prop && prop.name && prop.name.getText() === propName && prop.initializer) {
          opts.warn(
            `${sourceFileName}:${lineAndCharacter.line}:${lineAndCharacter.character} - ${propName}
            signature has changed; DOM event is now passed as first argument.`
          );
        }
      }
    }
